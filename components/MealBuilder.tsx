...
export function MealBuilder({ weekStart }: { weekStart: string }) {
  ...
  // Compute totals - calculate per-unit nutrition first
  ...
  const searchResults = useMemo(() => {
    ...
    return allGroceries
      .filter(
        (g) =>
          g.foodName.toLowerCase().includes(query) &&
          !ingredients.some((i) => i.groceryId === g.id),
      )
      .slice(0, 6);
  }, [searchQuery, allGroceries, ingredients]);

  const addIngredient = useCallback(
    async (grocery: GroceryItem) => {
      ...
      // If this is from API search (not a real grocery), create it first
        ...
        try {
          const response = await fetch("/api/groceries", {
            ...
            body: JSON.stringify({
              ...
              unit: normalizeUnit(grocery.unit),
          ...
          if (!response.ok) {
            ...
            console.error("Grocery creation error:", errorData);
  ...
  const updateQuantity = useCallback((rowId: string, newQuantity: number) => {
    setIngredients((prev) =>
      prev.map((ing) =>
        ing.rowId === rowId
          ? { ...ing, quantityUsed: Math.max(0.1, newQuantity) }
          : ing,
      ),
    );
  }, []);
      ...
      >
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: "0.6rem", color: "#999999", margin: 0 }}>
              cal
            </div>
          </div>
          ...
          >
            <div
              style={{ fontSize: "0.7rem", color: "#8B7FB8", fontWeight: 700 }}
            >
          ...
          >
            <div
              style={{ fontSize: "0.7rem", color: "#D67BB8", fontWeight: 700 }}
            >
          ...
          >
            <div
              style={{ fontSize: "0.7rem", color: "#C9845F", fontWeight: 700 }}
            >
        ...
        >
          ...
          <div style={{ marginLeft: "1.5rem", flex: 1 }}>
            ...
            >
              ...
              >
                <span style={{ color: "#8B7FB8", fontWeight: 600 }}>
                  Protein
                </span>
                <span style={{ color: "#2C2C2A", fontWeight: 700 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "0.75rem", color: "#999999" }}>
                      {result.totalCalories
                        ...
                        : 0}{" "}
                      cal | {result.quantityBought} {result.unit}
                    </div>
                  </div>
                  <button
          ...
          {showApiSearch && (
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => setShowApiSearch(false)}
                  style={{
                    background: "none",
                    border: "none",
                    color: "#C9845F",
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: "0.75rem", color: "#999999" }}>
                          {result.totalCalories
                            ? Math.round(result.totalCalories)
                            : 0}{" "}
                          cal | P: {result.proteinG || 0}g | C:{" "}
                          {result.carbsG || 0}g | F: {result.fatG || 0}g
                        </div>
                      </div>
                      <button
                        onClick={() => addIngredient(result)}
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ fontSize: "0.6rem", color: "#999999" }}>
                        cal
                      </div>
                    </div>
                    <div
